...
export default function PledgeModal({ isOpen, onClose, onConfirm }) {
      ...
      <div className="bg-white w-full max-w-sm rounded-2xl p-6 shadow-2xl animate-in">
        ...
        <div className="space-y-3 mb-6 text-sm text-gray-700">
          {[
            ...
            <label key={i} className="flex items-start space-x-2 p-3 bg-gray-50 rounded-lg border border-gray-200 cursor-pointer hover:bg-green-50 transition-colors">
              <input 
                type="checkbox" 
                checked={checks[i]}
        ...
        <button 
          ...
          className="w-full bg-green-600 text-white font-bold py-3 rounded-xl hover:bg-green-700 mb-2 transition-colors shadow-md"
        >
          서약 완료하기 (+50P)
        </button>
        <button 
